...
export interface HeartbeatStarterOptions {
  ...
  intervalMs?: number;
  /**
   * Optional logger — when provided, every heartbeat is reported via
   * `logger.beat(...)` (verbose-only by design, so default verbosity
   * doesn't see the 30s spam).
   */
  logger?: Logger;
}
...
export function startHeartbeat(opts: HeartbeatStarterOptions): HeartbeatHandle {
  ...
  const timer = setInterval(() => {
    ...
    try {
      opts.send(params);
      if (opts.logger) {
        // beat() is internally gated to verbose-only, so this is safe
        // to call every interval without flooding default output.
        opts.logger.beat(
          `heartbeat cpu=${params.stats.cpuPct ?? '?'}% mem=${memMB}MB inflight=${params.stats.inflightInvocations}`,
        );
      }
    } catch (err) {
      ...
      if (opts.logger) {
        opts.logger.error('heartbeat send failed', err);
      } else {
