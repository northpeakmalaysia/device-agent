...
Usage:
  ...
  ${PROG} run [--server <URL>] [--config <path>]
  ${PROG} status
  ${PROG} rotate
  ${PROG} forget
  ${PROG} --help | -h
...
async function cmdRun(flags: Record<string, string | true>): Promise<number> {
  ...
  if (!cfg) {
    process.stderr.write(
      `${PROG}: not paired. Run \`${PROG} pair --server <URL> --pair-code <CODE>\` first.\n` +
        `       (Looked for config at: ${configPath})\n`,
    );
    return 5;
  }
  ...
  if (pluginLoad.missing.length > 0) {
    process.stderr.write(
      `${PROG}: warning — tool plugins not found: ${pluginLoad.missing.join(', ')}.\n` +
        `       The daemon will run with whatever tools loaded (${pluginLoad.loaded.length}).\n` +
        `       Install missing plugins with \`npm install -g ${pluginLoad.missing.join(' ')}\` or via the bundled distribution.\n`,
    );
  }
  ...
  try {
    ...
    process.stderr.write(
      `${PROG}: cannot start — \`@swarmai/tools\` is not installed in this environment.\n` +
        `       Underlying error: ${err instanceof Error ? err.message : err}\n` +
        `       Run inside the bundled distribution at \`F:/Published/SwarmAI/\` (or wherever your operator installed it), or \`npm install -g @swarmai/tools\` once the package is published.\n`,
    );
    return 6;
  }
  ...
  console.log(
    `[device-agent] starting (device=${effective.deviceId}, platform=${effective.platform}, tools=${catalog.length})`,
  );
  ...
  const shutdown = async (sig: NodeJS.Signals) => {
    console.log(`[device-agent] received ${sig}, shutting down…`);
    try {
      await conn.close();
    } catch (err) {
      console.warn(
        `[device-agent] shutdown error: ${err instanceof Error ? err.message : err}`,
      );
    }
  };
  ...
  await conn.done;
  console.log(`[device-agent] exited`);
  return 0;
}
